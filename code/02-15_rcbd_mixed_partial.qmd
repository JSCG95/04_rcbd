...
```{r setup}
# Loading packages
## Double check Matrix package installation. 
...
That is because `lm()` only handles one variance component (the error itself) --> Only allows for the accomodation of a single source of variation (error). 

To account for more than one variance component (i.e., random effects (blocks) and error), we can use function `lmer()` from package `lme4`.  
...
# f) ANOVA table  
When we call ANOVA, no line for random effects. Is only giving fixed effect size of the model only. 

Difference treating as fixed --> 
...
Remember --> augment() creates the residuals for us. broom mixed adressed the issue with mix models. 
Here stundentized residuals are not given by default (Notice: .studresid=rstudent(rcbd_mix_mod))
```{r rcbd_mix_resid}
library(broom.mixed)
...
Warning: Flat line indicates lack of variability across my random effect.
...
rcbd_mix_means_all
```
kenward-roger to approximate df as denominator for calculation is more complex (2 sources of error now --> block and error)
