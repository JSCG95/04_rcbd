...
```
# c) Wrangling
Here rep column is more important than with crd
```{r rcbd_dfw}
rcbd_dfw <- rcbd_df %>%
...
Levels of one of the variables are different across levels of the other variable. 
Magnitude of how is increasing could mean that there in fact is an interaction. 
# e) Statistical model  
## Set-to-zero vs. sum-to-zero  
In R, the default contrast type is set-to-zero. Not what we want when running analysis of variance. 
...
We are not adding interaction between rep and other treatment factors. Thats how it is for RCBD
...
Remember that we are running our blocks as fixed effects.
...
NOTICE: We have rep here as part of the ANOVA. This represents the blocks.
Show always first ANOVA table to inform about the significant effects, to know why you are showing specific plots. 
...
- For data coming from an observational study, we need to be more severe with the interpretation. 
```{r }
...
None adjustment --> Fisher least significant difference (in papers). --> Using ANOVA as our filter.
Adjustment --> Make even more strict, more difficult to find difference. With none we are not strict at all.
We have an argument called alpha to increase the likelihood of spotting differences among treatments (the higher the alpha the easier will be to find differences). 
...
Everything is going to be apply to every geom. 
Default geom jitter gets color and not fill. Shape 21 has color and fill.
REMEMBER: Geom labels to add 
```{r rcbd final plot}

# Fill of every geom according to trtname. 
