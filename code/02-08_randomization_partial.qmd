...
Get all factor combinations (treatments)
Also creates a treatment number in another column (nrow(.))
```{r}
...
ggplot(data = crd_layout,
  ...
  labs(title = "CRD Randomization \n 3x3 Factorial (9 Treatments)") +
  theme_void()
...
# RCBD  
## Randomization  
Here rep number is important (is the block number)
```{r rcbd_rand}
# Setting seed to get same randomization result every time
set.seed(930)

## ARRANGE BY REP for RCBD
...
Tally() --> Reps for each combination. 
```{r}
rcbd_rand %>%
...
LEFT JOIN --> Merging datasets with common columns (in this case trtname). If they have different row numbers it will repeat the info based on trtname column. 
```{r}
rcbd_rand %>% 
